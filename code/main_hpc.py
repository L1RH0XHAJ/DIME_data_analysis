...
# print(OUTPUT_1['later_than_special'].value_counts())
# print(f"Types of election for later_than_special == 0 (total observations: {len(OUTPUT_1[OUTPUT_1['later_than_special'] == 0])})")
# print(OUTPUT_1[OUTPUT_1['later_than_special'] == 0]['election.type'].value_counts())
# print(f"Types of election for later_than_special == 1 (total observations: {len(OUTPUT_1[OUTPUT_1['later_than_special'] == 1])})")
# print(OUTPUT_1[OUTPUT_1['later_than_special'] == 1]['election.type'].value_counts())
# print()

# # Creating variable for election.type == 'S'
# OUTPUT_1['election_type_S'] = 0
# OUTPUT_1.loc[OUTPUT_1['election.type'] == 'S', 'election_type_S'] = 1

# print("Value counts for election_type_S:")
# print(OUTPUT_1['election_type_S'].value_counts())
# print()
# print("Contributions going to special elections before special elections date:", len(OUTPUT_1[(OUTPUT_1['election_type_S'] == 1) & (OUTPUT_1['date'] < OUTPUT_1['spec_election_date'])]),
#       f"(Percent of total: {round(100 * len(OUTPUT_1[(OUTPUT_1['election_type_S'] == 1) & (OUTPUT_1['date'] < OUTPUT_1['spec_election_date'])]) / len(OUTPUT_1[OUTPUT_1['election_type_S'] == 1]), 2)}%)")
# print("Contributions going to special elections on special elections date:", len(OUTPUT_1[(OUTPUT_1['election_type_S'] == 1) & (OUTPUT_1['date'] == OUTPUT_1['spec_election_date'])]),
#       f"(Percent of total: {round(100 * len(OUTPUT_1[(OUTPUT_1['election_type_S'] == 1) & (OUTPUT_1['date'] == OUTPUT_1['spec_election_date'])]) / len(OUTPUT_1[OUTPUT_1['election_type_S'] == 1]), 2)}%)")
# print("Contributions going to special elections after special elections date:", len(OUTPUT_1[(OUTPUT_1['election_type_S'] == 1) & (OUTPUT_1['date'] > OUTPUT_1['spec_election_date'])]),
#       f"(Percent of total: {round(100 * len(OUTPUT_1[(OUTPUT_1['election_type_S'] == 1) & (OUTPUT_1['date'] > OUTPUT_1['spec_election_date'])]) / len(OUTPUT_1[OUTPUT_1['election_type_S'] == 1]), 2)}%)")
...
district_cycle_deaths = OUTPUT_1[OUTPUT_1['spec_member'].notna()][
    ['district', 'cycle', 'death_date']].drop_duplicates()

# We calculate days to nearest death for all rows, and the absolute value as well
OUTPUT_1['days_to_nearest_death'] = (OUTPUT_1['date'] - OUTPUT_1['death_date']).dt.days
OUTPUT_1['abs_days_to_death'] = OUTPUT_1['days_to_nearest_death'].abs()
...
# print("Nr of districts with contributions that either have later_than_special == 0:", OUTPUT_1[OUTPUT_1['later_than_special'] == 0]['district'].nunique())
# print("Nr of districts with contributions that either have later_than_special == 1:", OUTPUT_1[OUTPUT_1['later_than_special'] == 1]['district'].nunique())
# print("Nr of districts with contributions that either have later_than_special == 1 or == 0:", OUTPUT_1[(OUTPUT_1['later_than_special'] == 0) | (OUTPUT_1['later_than_special'] == 1)]['district'].nunique())
# print("Total nr of districts in data that have special election:", OUTPUT_1[~OUTPUT_1['spec_election_date'].isna()]['district'].nunique())
# print("Total nr of districts in data:", OUTPUT_1['district'].nunique())
...
#     print(OUTPUT_1[(OUTPUT_1['district'] == district) & (OUTPUT_1['days_to_nearest_death'].notna())][['cycle', 'date', 'amount', 'death_date', 'days_to_nearest_death']].sort_values('date').head(10))
#     print("\nLast 10 contributions:")
#     print(OUTPUT_1[(OUTPUT_1['district'] == district) & (OUTPUT_1['days_to_nearest_death'].notna())][['cycle', 'date', 'amount', 'death_date', 'days_to_nearest_death']].sort_values('date').tail(10))
#     print()
#     print("=" * 20)
#     print()
...
# print(OUTPUT_1['days_to_nearest_death'].describe())

# plt.figure(figsize=(10, 6))
# sns.histplot(data=OUTPUT_1, x='days_to_nearest_death', bins=30)
# plt.axvline(x=0, color='red', linewidth=3, label = 'Difference = 0')
# plt.axvline(x=OUTPUT_1['days_to_nearest_death'].mean(), color='blue', linewidth=3, label = 'Mean value')  # adds vertical line
...
for district in single_death_districts + multiple_death_districts:
    ...
    election_dates = OUTPUT_1['election_date_in_cycle'].dropna().unique()
    
    # For each contribution in the district
    district_mask = OUTPUT_1['district'] == district
    
    if district in single_death_districts:
        print(f"{district} is single death")
        OUTPUT_1.loc[district_mask, 'treat_2'] = OUTPUT_1.loc[district_mask, 'treat_1']
        
    elif district in multiple_death_districts:
        print(f"{district} is multiple death")
        for idx in OUTPUT_1[district_mask].index:
            contrib_date = OUTPUT_1.loc[idx, 'date']
            ...
            OUTPUT_1.loc[idx, 'treat_2'] = state        
...
#     OUTPUT_1_sample = OUTPUT_1[OUTPUT_1['district'] == picked_district]
#     death_dates = OUTPUT_1_sample['death_date'].dropna().unique()
    
#     # Define election_dates here so it's available in both branches
#     election_dates = OUTPUT_1_sample['election_date_in_cycle'].dropna().unique()
    
#     if treatment == 'treat_1':
#         # Plot for treat_1
#         plt.figure(figsize=(10, 6))
#         plt.scatter(OUTPUT_1_sample['date'], OUTPUT_1_sample['treat_1'], c='blue', label = 'Contributions', s = 0.5)
...
#         plt.figure(figsize=(10, 6))
#         plt.scatter(OUTPUT_1_sample['date'], OUTPUT_1_sample['treat_2'], c='red', label = 'Contributions', s = 0.5)
#         for i, death_date in enumerate(death_dates):
#             plt.axvline(x=death_date, color='purple', linewidth=3, 
...
#     print(OUTPUT_1[
#         (OUTPUT_1['district'] == district) & (OUTPUT_1['days_to_nearest_death'].notna())
...
# dropped_indices = set(OUTPUT_1.index) - set(OUTPUT_1.index)
# dropped_rows = OUTPUT_1.loc[list(dropped_indices)]    
# dropped_rows_CA05 = dropped_rows[dropped_rows['district'] == 'CA05']

# # Using duplicates function
# duplicates = OUTPUT_1[OUTPUT_1.duplicated(subset=['transaction.id', 'district'], keep=False)]
